...
/**
 * Question behaviour for auc_responses
 *
...
class qbehaviour_auc_responses extends question_behaviour_with_multiple_tries {
    ...
    public function adjust_display_options(question_display_options $options) {
        ...
        if (!$this->is_try_again_state()) {
            ...
            if (
                $this->qa->get_state() == question_state::$invalid &&
        ...
        // The question is in a try-again state. Let the renderer know this.
        $options->readonly = $options->readonly
            ? self::TRY_AGAIN_VISIBLE_READONLY
    ...
    public function get_applicable_hint() {
        if (!$this->is_try_again_state() || empty($this->question->hints)) {
            return null;
        }
        ...
        $hintindex = $totaltries - $triesleft - 1;

        if ($hintindex < 0 || $hintindex >= count($this->question->hints)) {
            return null;
        }

        return $this->question->get_hint($hintindex, $this->qa);
    }
    ...
    public function init_first_step(question_attempt_step $step, $variant) {
        ...
        if (property_exists($this->question, 'answers') && is_array($this->question->answers)) {
            $step->set_behaviour_var('_triesleft', count($this->question->answers));
        } else {
